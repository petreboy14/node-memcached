...
function Client(args, options){
  if(!(this && this.hasOwnProperty && (this instanceof Client))) {
    this = new Client();
  }
...
(function(nMemcached){
  ...
  memcached.connect = function connect (server, callback){
    ...
    if (server in this.issues && this.issues[server].failed) {
      return callback(new Error('Failed to connect to the server'));
    }
    ...
    this.connections[server] = new Manager(server, this.poolSize, function manager (callback) {
      var stream = new Stream
        , Manager = this;

      // config the Stream
      stream.streamID = sid++;
      stream.setTimeout(memcached.timeout);
      stream.setNoDelay(true);
      stream.metaData = [];
      stream.responseBuffer = "";
      stream.bufferArray = [];
      stream.serverAddress = server;
      stream.tokens = serverTokens;
      stream.memcached = memcached;
      ...
      Utils.fuse(stream, {
        ...
        , error: function streamError (err) {
            memcached.connectionIssue(err, stream, callback)
          }
        , data: Utils.curry(memcached, private.buffer, stream)
      ...
      stream.connect.apply(stream, serverTokens);
      return stream;
    });
  ...
  memcached.command = function memcachedCommand (queryCompiler, server) {
    ...
    if (server in this.issues && this.issues[server].failed) {
      return query.callback
        && query.callback(new Error('Failed to connect to the server'));
    }

    this.connect(server, function allocateConnection (error, stream) {
      if (Client.config.debug) {
        query.command.split(LINEBREAK).forEach(function each (line) {
          console.error(stream.streamID + ' \033[34m<<\033[0m ' + line);
        });
      }

      // check for errors that might be generated during the connection
      if (error) return query.callback && query.callback(error);

      // check if we actually have a stream to write to
      if (!stream) {
        return query.callback
          && query.callback(new Error('The connect method does not have a working connection'));
      }

      // check if the stream is workable
      if (stream.readyState !== 'open') {
        return query.callback
          && query.callback(new Error('Connection readyState is set to ' + stream.readySate));
      }

      // used for request timing
      query.start = Date.now();
      stream.metaData.push(query);
      stream.write(query.command + LINEBREAK);
    });
    ...
    if (redundancy && queryRedundancy){
      ...
      redundancy.forEach(function each (server) {
        ...
        memcached.connect(server, function allocateConnection (error, stream) {
          if (!stream || error || stream.readyState !== 'open') return;

          stream.write(queryRedundancy.command + LINEBREAK);
        });
      })
    }
  };
  ...
  // cache misses.
  memcached.connectionIssue = function connectionIssue (error, stream, callback) {
  ...
  memcached.end = function endMemcached () {
    ...
    Object.keys(this.connections).forEach(function close (key){
  ...
  private.parsers = {
    ...
    , 'CLIENT_ERROR': function clientfail (tokens, dataSet, err) {
        err.push(tokens.splice(1).join(' '));
        return [CONTINUE, false]
      }

    , 'SERVER_ERROR': function serverfail (tokens, dataSet, err, queue, stream, memcached) {
        (memcached || this.memcached).connectionIssue(tokens.splice(1).join(' '), stream);
        return [CONTINUE, false]
      }
    ...
    , 'OK': function okidoki (tokens, dataSet) {
        return [CONTINUE, true]
      }

    , 'EXISTS': function IHAS (tokens, dataSet) {
        return [CONTINUE, false]
      }
    ...
    , 'STAT': function stats (tokens, dataSet, err, queue) {
  ...
  // Generates a RegExp that can be used to check if a chunk is memcached
  // response identifier
  ...
  // and user our buffered buffer to query against. Also when you execute allot
  // of .writes to the same stream, node will combine the responses in to one
  // response stream. With no indication where it had cut the data. So it can be
  // it cuts inside the value response, or even right in the middle of
  ...
  private.buffer = function BufferBuffer(stream, BufferStream){
    stream.responseBuffer += BufferStream;

    // only call transform the data once we are sure, 100% sure, that we valid response ending
    if (stream.responseBuffer.substr(stream.responseBuffer.length - 2) === LINEBREAK) {
      var chunks = stream.responseBuffer.split(LINEBREAK);

      if (Client.config.debug) {
        chunks.forEach(function each (line) {
          console.error(stream.streamID + ' \033[35m>>\033[0m ' + line);
        });
      }

      stream.responseBuffer = ""; // clear!
      this.rawDataReceived(stream, stream.bufferArray = stream.bufferArray.concat(chunks));
    }
  };
  ...
  memcached.rawDataReceived = function rawDataReceived (stream){
    var queue = []
      , token
      , tokenSet
      , dataSet = ''
    ...
    while (stream.bufferArray.length && private.allCommands.test(stream.bufferArray[0])) {

      token = stream.bufferArray.shift();
      ...
      if (tokenSet[0] == 'VALUE' && stream.bufferArray.indexOf('END') == -1) {
        return stream.bufferArray.unshift(token);
      }
      ...
      if (private.parsers[tokenSet[0]]) {
        ...
        if (tokenSet[0] == 'VALUE') {
          while (stream.bufferArray.length) {
            if (private.bufferedCommands.test(stream.bufferArray[0])) break;

            dataSet += stream.bufferArray.shift();
          };
        }

        resultSet = private.parsers[tokenSet[0]].call(stream, tokenSet, dataSet || token, err, queue, this);
        ...
        switch (resultSet.shift()) {
          ...
          case FLUSH:
            metaData = stream.metaData.shift();
            resultSet = queue;
            ...
            if (metaData && metaData.callback) {
              ...
              metaData.callback.call(
                ...
                private.resultParsers[metaData.type]
                  ? private.resultParsers[metaData.type].call(stream, resultSet, err)
      ...
      // item at the end..
      if (stream.bufferArray[0] === '') stream.bufferArray.shift();
    };
  };
  ...
  private.setters = function setters (type, validate, key, value, lifetime, callback, cas) {
    ...
    if (length > memcached.maxValue) {
      return private.errorResponse(new Error('The length of the value is greater than ' + memcached.maxValue), callback);
    }
